...
// Fonction récursive pour calculer les fuites et trouver le pire tronçon
double calculer_fuites_recursif_avec_pire(NoeudArbre *noeud, PireTroncon *pire) {
    if (noeud == NULL) {
        return 0;
    }
    ...
    // Conversion en M.m3 
    double fuite_locale_mm3 = fuite_locale / 1000.0;
    
    // Vérifier si ce tronçon est le pire 
    if (fuite_locale_mm3 > pire->volume_fuite) {
        pire->volume_fuite = fuite_locale_mm3;
    }
    ...
    ListeEnfants *temp = noeud->enfants;
    while (temp != NULL) {
        fuites_enfants += calculer_fuites_recursif_avec_pire(temp->enfant, pire);
        temp = temp->suivant;
    }
...
// Fonction pour trouver le pire tronçon en parcourant l'arbre
void trouver_pire_troncon_recursif(NoeudArbre *noeud, PireTroncon *pire) {
    if (noeud == NULL) {
        return;
    }
    ...
    ListeEnfants *temp = noeud->enfants;
    while (temp != NULL) {
        NoeudArbre *enfant = temp->enfant;
        
        // Fuite sur le tronçon noeud -> enfant
        double fuite_troncon = enfant->volume_entrant * (enfant->pourcentage_fuite / 100.0);
        double fuite_troncon_mm3 = fuite_troncon / 1000.0;
        
        if (fuite_troncon_mm3 > pire->volume_fuite) {
            pire->volume_fuite = fuite_troncon_mm3;
            
            // Libérer les anciens identifiants s'ils existent
            if (pire->identifiant_amont != NULL) {
                free(pire->identifiant_amont);
            }
            if (pire->identifiant_aval != NULL) {
                free(pire->identifiant_aval);
            }
            
            // Copier les nouveaux identifiants
            pire->identifiant_amont = malloc(strlen(noeud->identifiant) + 1);
            strcpy(pire->identifiant_amont, noeud->identifiant);
            
            pire->identifiant_aval = malloc(strlen(enfant->identifiant) + 1);
            strcpy(pire->identifiant_aval, enfant->identifiant);
        }
        
        trouver_pire_troncon_recursif(enfant, pire);
        
        temp = temp->suivant;
    }
}

// Calculer les fuites totales et identifier le pire tronçon
double calculer_fuites_avec_pire_troncon(NoeudArbre *racine, PireTroncon *pire) {
    if (racine == NULL) {
        return -1;
    }
    
    pire->identifiant_amont = NULL;
    pire->identifiant_aval = NULL;
    pire->volume_fuite = 0.0;
    
    propager_volume(racine);
    
    trouver_pire_troncon_recursif(racine, pire);
