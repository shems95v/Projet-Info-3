...
if [ "$COMMAND" = "histo" ]; then
    ...
    if [ "$#" -ne 3 ]; then
        echo "Erreur: Usage: $0 <fichier_donnees> histo <max|src|real|all>" >&2
        afficher_temps
        exit 1
    fi

    # Vérification du mode
    if [ "$MODE" != "max" ] && [ "$MODE" != "src" ] && [ "$MODE" != "real" ] && [ "$MODE" != "all" ]; then
        echo "Erreur: Le mode doit être 'max', 'src', 'real' ou 'all'." >&2
        afficher_temps
        exit 1
    fi
    ...
    if [ "$MODE" = "max" ]; then
        ...
        TITLE_REF="Vue Complète des Volumes (Max, Source, Réel)"
    fi
    ...
    if [ "$MODE" = "max" ]; then
        ...
        # Pour le mode all, tri par la colonne 2 (volume max)
        tail -n +2 "$OUTPUT_FILE" | sort -t ';' -k 2,2gr | head -n 10 >> "${MODE}_10_plus_grandes_usines.dat"
        tail -n +2 "$OUTPUT_FILE" | sort -t ';' -k 2,2g | head -n 50 >> "${MODE}_50_plus_petites usines.dat"
    else
...
EOF
    ...
    # Fonction pour générer un script Gnuplot pour le mode "all" (3 couleurs)
    generate_gnuplot_script_all() {
        local DATA_IN=$1
        local PNG_OUT=$2
        local CHART_TITLE=$3
        local X_LABEL="Usine"
...
# Légende : Bleu = Volume réel, Rouge = Pertes après captage, Vert = Capacité restante
plot '$DATA_IN' using 4:xtic(1) title "Volume réel (traité)" linecolor rgb "#4472C4", \\
     '' using (\$3-\$4) title "Pertes après captage" linecolor rgb "#C44444", \\
     '' using (\$2-\$3) title "Capacité restante" linecolor rgb "#44C444"
EOF
    }
    ...
    if [ "$MODE" = "all" ]; then
        # Utiliser la fonction spéciale pour le mode all
        generate_gnuplot_script_all "${MODE}_10_plus_grandes_usines.dat" "$PNG_10" "$TITLE_10"
    else
        # Utiliser la fonction normale pour les autres modes
    ...
    if [ "$MODE" = "all" ]; then
        # Utiliser la fonction spéciale pour le mode all
        generate_gnuplot_script_all "${MODE}_50_plus_petites usines.dat" "$PNG_50" "$TITLE_50"
    else
        # Utiliser la fonction normale pour les autres modes
    ...
    # Exécution du programme C et capture de toute la sortie
    OUTPUT=$("$EXECUTABLE" "$DATA_FILE" leaks "$USINE_ID" 2>&1)
    EXIT_CODE=$?
    
    # Extraction de la première ligne (volume total)
    ...
    if [ "$RESULT" = "-1.000000" ] || [ $EXIT_CODE -eq 4 ]; then
        ...
        # Affichage des informations du pire tronçon
        PIRE_TRONCON=$(echo "$OUTPUT" | grep "Pire tronçon:")
        if [ ! -z "$PIRE_TRONCON" ]; then
            echo "$PIRE_TRONCON"
        fi
    fi
