...
void avl_parcours_inverse_all(NoeudAVL *noeud, FILE *sortie) {
    ...
    avl_parcours_inverse_all(noeud->droite, sortie);

    Usine *u = noeud->usine;
    
    // Conversion de k.m3 en M.m3
    double max_volume = u->volume_max / 1000.0;
    double source_volume = u->volume_source / 1000.0;
    double reel_volume = u->volume_reel / 1000.0;
    
    // Écriture des 4 colonnes 
    fprintf(sortie, "%s;%.6f;%.6f;%.6f\n", u->identifiant, max_volume, source_volume, reel_volume);

    
    avl_parcours_inverse_all(noeud->gauche, sortie);
}